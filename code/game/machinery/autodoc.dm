...
//	var/surgery_t = 0 //Surgery timer in seconds.
	...
	var/obj/item/weapon/reagent_containers/blood/OMinus = new()
	var/filtering = 0
...
/obj/machinery/autodoc/process()
	...
	if(occupant)
		...
		if(surgery)
			...
			if(blood_transfer)
				if(occupant.blood_volume < BLOOD_VOLUME_BAD)
					if(blood.reagents.get_reagent_amount("blood") < 4)
						blood.reagents.add_reagent("blood", 195, list("donor"=null,"viruses"=null,"blood_DNA"=null,"blood_type"="O-","resistances"=null))
						visible_message("\ [src] speaks: Blood reserves depleted, switching to fresh bag.")
					occupant.inject_blood(blood, 8) // double iv stand rate
					if(prob(10))
...
/datum/autodoc_surgery
	var/obj/item/organ/external/limb_ref = null
	var/obj/item/organ/internal/organ_ref = null
...
/proc/generate_autodoc_surgery_list(mob/living/carbon/human/M)
	...
	var/known_implants = list(/obj/item/weapon/implant/chem, /obj/item/weapon/implant/death_alarm, /obj/item/weapon/implant/loyalty, /obj/item/weapon/implant/tracking, /obj/item/weapon/implant/adrenalin)
	for(var/obj/item/organ/external/L in M.limbs)
		if(L)
			for(var/obj/item/organ/external/affecting/W in L.wounds)
				if(W.internal)
					surgery_list += create_autodoc_surgery(L,LIMB_SURGERY,ADSURGERY_INTERNAL)
					break

			var/organdamagesurgery = 0
			for(var/obj/item/organ/internal/I in L.internal_organs)
				...
				if(I.damage > 0)
					if(I.organ_id == O_EYES) // treat eye surgery differently
						continue
			...
			if(L.implants.len)
				for(var/I in L.implants)
					if(!is_type_in_list(I,known_implants))
						...
						if(L.body_part == BP_CHEST)
							skip_embryo_check = TRUE
			var/obj/item/alien_embryo/A = locate() in M
			if(A && L.body_part == BP_CHEST && !skip_embryo_check) //If we're not already doing a shrapnel removal surgery on the BP_CHEST, add an extraction surgery to remove it
				surgery_list += create_autodoc_surgery(L,LIMB_SURGERY,ADSURGERY_SHRAPNEL)
	...
	var/obj/item/organ/internal/I = M.internal_organs_by_name["eyes"]
			...
			if(ORGAN_SURGERY)
				switch(S.surgery_procedure)
					...
					if(ADSURGERY_DAMAGE)
						...
						if(S.limb_ref.body_part != BP_GROIN)
							open_encased(H,S.limb_ref)

						if(!istype(S.organ_ref,/obj/item/organ/internal/brain))
						...
						if(!surgery) break
						if(istype(S.organ_ref,/obj/item/organ/internal))
							S.organ_ref.rejuvenate()
						else
							visible_message("\ [src] speaks, Organ is missing.");

						// close them
						if(S.limb_ref.body_part != BP_GROIN) // TODO: fix brute damage before closing
					...
					if(ADSURGERY_EYES)
						...
						if(S.unneeded)
							...
							surgery_todo_list -= S
							continue
						if(istype(S.organ_ref,/obj/item/organ/internal/eyes))
							var/obj/item/organ/internal/eyes/E = S.organ_ref
			...
			if(LIMB_SURGERY)
				switch(S.surgery_procedure)
					...
					if(ADSURGERY_BROKEN)
						...
						if(!surgery) break
						S.limb_ref.limb_status &= ~ORGAN_BROKEN
					...
					if(ADSURGERY_MISSING)
						...
						if(S.limb_ref.parent.limb_status & ORGAN_DESTROYED) // there's nothing to attach to
					...
					if(ADSURGERY_NECRO)
						...
						S.limb_ref.limb_status &= ~ORGAN_DEAD
						H.update_body()
					...
					if(ADSURGERY_SHRAPNEL)
						...
						open_incision(H,S.limb_ref)
						if(S.limb_ref.body_part == BP_CHEST || S.limb_ref.body_part == BP_HEAD)
							open_encased(H,S.limb_ref)
						if(S.limb_ref.body_part == BP_CHEST) //if it's the BP_CHEST check for gross parasites
					...
					if(ADSURGERY_FACIAL) // dumb but covers for incomplete facial surgery
						...
						if(S.unneeded)
							...
							surgery_todo_list -= S
							continue
						if(istype(S.limb_ref,/obj/item/organ/external/head))
							var/obj/item/organ/external/head/F = S.limb_ref
...
/obj/machinery/autodoc/proc/open_incision(mob/living/carbon/human/target, var/obj/item/organ/external/L)
...
/obj/machinery/autodoc/proc/close_incision(mob/living/carbon/human/target, var/obj/item/organ/external/L)
...
/obj/machinery/autodoc/proc/open_encased(mob/living/carbon/human/target, var/obj/item/organ/external/L)
	if(target && L && L.surgery_open_stage >= 2)
...
/obj/machinery/autodoc/proc/close_encased(mob/living/carbon/human/target, var/obj/item/organ/external/L)
	if(target && L && L.surgery_open_stage > 2)
...
/obj/machinery/autodoc/verb/move_inside()
	...
	if(do_after(usr, 10, FALSE, 5, BUSY_ICON_GENERIC))
		...
		occupant = usr
		icon_state = "autodoc_closed"
		var/implants = list(/obj/item/weapon/implant/chem, /obj/item/weapon/implant/death_alarm, /obj/item/weapon/implant/loyalty, /obj/item/weapon/implant/tracking, /obj/item/weapon/implant/adrenalin)
...
/obj/machinery/autodoc/attackby(obj/item/W, mob/living/user)
	...
	if(istype(W, /obj/item/weapon/grab))
		...
		if(ismob(G.grabbed_thing))
			M = G.grabbed_thing
		else if(istype(G.grabbed_thing,/obj/item/bodybag/cryobag))
			var/obj/item/bodybag/cryobag/C = G.grabbed_thing
			if(!C.stasis_mob)
	...
	if((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))))
		...
		if(connected.occupant && ishuman(connected.occupant))
			...
			if(href_list["organdamage"])
				for(var/obj/item/organ/external/L in connected.occupant.limbs)
					if(L)
						for(var/obj/item/organ/internal/I in L.internal_organs)
							if(I.robotic == ORGAN_ASSISTED||I.robotic == ORGAN_ROBOT)
								// we can't deal with these
						...
						if(L.implants.len)
							for(var/I in L.implants)
								if(!is_type_in_list(I,known_implants))
									...
									if(L.body_part == BP_CHEST)
										skip_embryo_check = TRUE
						if(A && L.body_part == BP_CHEST && !skip_embryo_check) //If we're not already doing a shrapnel removal surgery of the BP_CHEST proceed.
							N.fields["autodoc_manual"] += create_autodoc_surgery(L,LIMB_SURGERY,ADSURGERY_SHRAPNEL)
							needed++
			...
			if(href_list["facial"])
				for(var/obj/item/organ/external/L in connected.occupant.limbs)
					if(L)
						if(istype(L,/obj/item/organ/external/head))
							var/obj/item/organ/external/head/J = L
